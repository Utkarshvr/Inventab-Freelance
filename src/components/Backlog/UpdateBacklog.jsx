...
import BacklogDataForm from "../BacklogDataForm/BacklogDataForm.jsx";
...
function BacklogHistoryModal(props) {
  ...
  const submitHistory = async (
    ...
    try {
      const { id } = backlogData;
      e.preventDefault();

      const currentDate = formatDateToIndianVersion(new Date());
  ...
  useEffect(() => {
    if (backlogData?.backlog_history) {
      setHistories(backlogData?.backlog_history.reverse());
    }
  }, [backlogData?.backlog_history]);
...
const UpdateBacklog = () => {
    ...
    const leads = async () => {
      try {
        ...
        const { data } = await axios.get(
          `projects/get/project/backlog/?id=${id}`
        );
        setLoading(false);
  ...
  return (
    <div>
      <PageTitle title="Update Backlog" />
        ...
        <div className="col-xl-12 col-lg-12">
          <div className="card">
            <div className="card-header flex gap-3 gap-sm-3 gap-md-0 gap-lg-0 flex-column flex-md-row flex-lg-row flex-sm-column">
              <h4 className="card-title">Update Backlog </h4>
