...
import ProjectDataForm from "../ProjectDataForm/ProjectDataForm.jsx";
...
function ProjectHistoryModal(props) {
  ...
  useEffect(() => {
    if (projectData?.project_history) {
      setHistories(projectData?.project_history.reverse());
    }
  }, [projectData?.project_history]);
...
const UpdateProject = () => {
  ...
  useEffect(() => {
    const leads = async () => {
      try {
        setLoading(true);
        const { data } = await axios.get(`projects/get/projects/?id=${id}`);
        setLoading(false);
