...
//             to={`/dashboard/engg/update-backlog/${row?.id}`}
//           >
//             {row?.backlog}
//           </Link>
//         );
...
const Backlog = () => {
  ...
  const columns = [
    {
      ...
      cell: (row) => {
        return (
          <Link
            className="text-center text-info dark_theme_text"
            to={`/dashboard/engg/update-backlog/${row?.id}?backlog=${row?.backlog}`}
          >
            {row?.backlog}
          </Link>
        );
  ...
  useEffect(() => {
    ...
    if (selectedEl?.value) {
      result = backlog.filter((backlogData) => {
        const userFullName = `${backlogData?.user?.first_name || ""} ${backlogData?.user?.last_name || ""}`.toLowerCase();
        switch (selectedEl?.value) {
          case "backlog":
            return backlogData?.backlog
              ?.toLowerCase()
  ...
  const exportAsCsv = (searchData) => {
    ...
    searchData.forEach((backlogData) => {
      const csvObj = {
        ...
        Priority: backlogData?.priority || "",
        Status: backlogData?.status || "",
        User: `${backlogData?.user?.first_name || ""} ${backlogData?.user?.last_name || ""}`,
      };
                ...
                <DataTable
                  ...
                  actions={
                    <div className="d-flex">
                      ...
                      <Link to="/dashboard/engg/grid-backlog">
                        <button className="bg-primary btn text-white mb-3 border-0 align-items-center ms-2 rounded-1">
                          Grid View 
                        </button>
                      </Link>
